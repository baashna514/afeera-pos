...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <form action="{{ route('permissions.update', $permission) }}" method="POST" class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-5">
        ...
        <div>
            ...
            <p class="text-[11px] text-slate-400 mt-1">Unique identifier used in code guards & route middleware (e.g. <code class="font-mono text-brand-600">sales.create</code>). Cannot be modified to prevent system errors.</p>
        </div>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                      class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:bg-white transition">{{ old('description', $permission->description) }}</textarea>
        </div>
