    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-black text-slate-800 flex items-center gap-2.5">
                <i class="fa-solid fa-[file-invoice-dollar] text-brand-600"></i>
        ...
        <form action="{{ route('vouchers.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <label id="lbl_receipt" class="p-4 rounded-xl border-2 border-brand-500 bg-brand-50/50 cursor-pointer flex items-center gap-3 transition">
                        <input type="radio" name="type" value="receipt" checked onchange="toggleVoucherParty('receipt')" class="w-4 h-4 text-brand-600 focus:ring-brand-500">
                        <div>
                            <span class="block text-xs font-black text-brand-900 uppercase">Cash Receipt</span>
                            <span class="text-[11px] text-brand-700 font-medium">Receive Money from Customer (Khata Credit)</span>
                        </div>
                    </label>
            ...
            <div id="div_customer">
                ...
                <select name="customer_id" id="customer_id" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition">
            ...
            <div id="div_vendor" class="hidden">
                ...
                <select name="vendor_id" id="vendor_id" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="voucher_date" id="voucher_date" value="{{ old('voucher_date', date('Y-m-d')) }}" required
                           class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition">
                </div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0.01" name="amount" id="amount" value="{{ old('amount') }}" required placeholder="0.00"
                           class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-black focus:outline-none focus:ring-2 focus:ring-brand-500 transition">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="payment_method" id="payment_method" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition">
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="3" placeholder="e.g. Partial cash payment received for invoice settling..."
                          class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition">{{ old('notes') }}</textarea>
            </div>
...
<script>
    function toggleVoucherParty(type) {
        ...
        if (type === 'receipt') {
            ...
            lblReceipt.className = 'p-4 rounded-xl border-2 border-brand-500 bg-brand-50/50 cursor-pointer flex items-center gap-3 transition';
