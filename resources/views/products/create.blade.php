            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <button type="button" onclick="generateBarcode()" class="text-[11px] font-bold text-brand-600 hover:underline flex items-center gap-1">
                ...
                <div>
                    ...
                    <select name="default_discount_type" id="default_discount_type" 
                        ...
                        <option value="percentage" {{ old('default_discount_type', 'percentage') == 'percentage' ? 'selected' : '' }}>Percentage (%)</option>
                        <option value="fixed" {{ old('default_discount_type') == 'fixed' ? 'selected' : '' }}>Fixed Amount (Rs.)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="default_discount_value" id="default_discount_value" value="{{ old('default_discount_value', '0.00') }}"
                           class="w-full px-4 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-500 focus:bg-white transition">
            ...
            <div class="border-t border-slate-100 pt-6 space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Assign initial stock directly across multiple warehouses. Total initial stock will calculate automatically.</p>
                    </div>
                ...
                <div class="overflow-x-auto border border-slate-200 rounded-xl bg-slate-50/50">
                    ...
                    <div id="noWarehouseStocksMsg" class="p-4 text-center text-xs text-slate-400">
                        No warehouse stock rows added yet. If left empty, initial stock is assigned to your <strong>Default Warehouse</strong>. Click <strong>"+ Add Warehouse Stock"</strong> to split stock across locations.
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl overflow-hidden p-6 space-y-4">
        ...
        <form onsubmit="saveQuickCategory(event)" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" id="qc_cat_desc" placeholder="Brief category notes..." class="w-full px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand-500 focus:outline-none">
            </div>
...
<div id="quickBrandModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl overflow-hidden p-6 space-y-4">
        ...
        <form onsubmit="saveQuickBrand(event)" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">Description (Optional)</label>
                <input type="text" id="qc_brand_desc" placeholder="Brief brand notes..." class="w-full px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand-500 focus:outline-none">
            </div>
            <div class="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
...
<script>
    ...
    function addWarehouseStockRow(data = null) {
        ...
        availableWarehousesList.forEach(w => {
            const selected = data && data.warehouse_id == w.id ? 'selected' : (w.is_default && !data ? 'selected' : '');
