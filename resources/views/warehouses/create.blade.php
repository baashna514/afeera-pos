    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                <i class="fa-solid fa-warehouse text-brand-600"></i> Add New Warehouse
            </h1>
        ...
        <form action="{{ route('warehouses.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="address" rows="3" placeholder="Full street address or location details..."
                          class="w-full px-3.5 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-brand-500 focus:outline-none transition font-medium">{{ old('address') }}</textarea>
            </div>

            <div class="pt-2 border-t border-slate-100 flex items-center gap-6">
                <label class="flex items-center gap-2 text-xs font-bold text-slate-700 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" checked class="w-4 h-4 rounded text-brand-600 focus:ring-brand-500 border-slate-300">
                    <span>Active Location</span>
                </label>

                <label class="flex items-center gap-2 text-xs font-bold text-slate-700 cursor-pointer">
                    <input type="checkbox" name="is_default" value="1" class="w-4 h-4 rounded text-brand-600 focus:ring-brand-500 border-slate-300">
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button type="submit" class="px-6 py-2.5 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl text-xs shadow-xs transition">Save Warehouse</button>
            </div>
        </form>
    </div>
</div>
