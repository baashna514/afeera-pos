...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-black text-slate-800">Register New Company</h2>
            <p class="text-xs text-slate-500 mt-0.5">Add a new organization/tenant to activate segregated inventory, sales, and staff access.</p>
        </div>
        ...
        <form action="{{ route('companies.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="currency" id="currency" value="{{ old('currency', 'PKR') }}"
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="address" id="address" rows="3"
                              ...
                              placeholder="Street address, city, region">{{ old('address') }}</textarea>
                    @error('address')
                        <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
                    @enderror
                </div>
