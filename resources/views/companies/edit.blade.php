    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
        <form action="{{ route('companies.update', $company) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <!-- Company Logo -->
                <div class="sm:col-span-2" x-data="{ logoPreview: '{{ $company->logo ? asset('storage/' . $company->logo) : '' }}' }">
                    ...
                    <div class="flex items-center gap-5 p-4 border border-dashed border-slate-300 rounded-2xl bg-slate-50/50 hover:bg-slate-50 transition">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-[11px] text-slate-400 mt-1">Upload company logo (PNG, JPG, WEBP, or SVG, max 2MB). Replaces previous logo on sidebar & invoices.</p>
                        </div>
                    </div>
                    @error('logo')
                        <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
